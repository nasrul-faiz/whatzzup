    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className={`flex-1 min-h-0 overflow-auto relative transition-all duration-200 ease-out ${viewModeTransition === "out" ? "opacity-0 scale-[0.98]" : "opacity-100 scale-100"}`}>
        {resources.length === 0 ? (
          ...
          <table className="border-collapse text-center w-full border border-border" style={{ tableLayout: "fixed" }}>
            <colgroup>
              ...
              {orderedResources.map((resource, ri) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* ── Staff cell ── */}
                    <td className="sticky left-0 z-10 border-r border-border px-2.5 py-2 align-middle bg-background" style={{ boxShadow: '2px 0 0 0 hsl(var(--border))' }}>
                      <div className="flex flex-col items-start gap-0.5 text-left">
                        <p className="max-w-full truncate text-[11px] font-semibold text-foreground leading-tight">{resource.name}</p>
                        {resource.role && (
                          <p className="max-w-full truncate text-[9px] text-muted-foreground leading-tight">{resource.role}</p>
                        )}
                        {showStaffShiftCount && (
                          <p className="mt-1 inline-flex rounded-full bg-muted/70 px-1.5 py-0.5 text-[9px] text-muted-foreground leading-tight">{rowShifts.length} shifts</p>
                        )}
                      </div>
                      {isEditMode && (
                        <div className="mt-2.5 flex items-center justify-start gap-1">
                          <button
                            ...
                            disabled={ri === 0}
                            className="flex h-6 w-6 items-center justify-center rounded-md border-0 bg-transparent text-muted-foreground/60 hover:bg-muted/65 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-20 transition-colors"
                            title="Move up"
                          >
                          ...
                          <button
                            ...
                            className="flex h-6 items-center gap-1 rounded-md border-0 bg-transparent px-1.5 text-[9px] font-medium text-red-500 transition-colors hover:bg-red-500/10 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300"
                          >
                    ...
                    {(() => {
                      ...
                      if (isEditMode) {
                        return dayInfos.map(({ dateKey, amShift, pmShift, offShift, isToday }) => (
                          <td
                            key={dateKey}
                            className={`border-r border-border align-middle transition-colors cursor-pointer hover:bg-muted/20 ${isToday ? "bg-primary/[0.04]" : ""}`}
                            style={{ width: `${dayColWidth}px`, minWidth: `${dayColWidth}px` }}
                      ...
                      return groups.map(group => {
                        ...
                        return (
                          <td
                            ...
                            className={`border-r border-border align-middle transition-colors ${todayInGroup ? "bg-primary/[0.04]" : ""}`}
