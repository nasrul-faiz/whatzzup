...
import {
  ...
  Navigation, Palette, Database, HardDrive, Clock3, Loader2, Smartphone, Sun, Moon,
} from "lucide-react"
...
export function Settings({ section = "profile" }: { section?: SectionId }) {
  const { appFont, setAppFont, theme, setMode } = useTheme()
  ...
  const t = {
    ...
    themeDescription: "Select light, dark, or system-based theme behavior.",
    themeLabel: "Theme",
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm space-y-6">
              <div className="grid gap-4 sm:grid-cols-3">
                {([
                  { value: "system", label: t.themeOptionSystem, description: "Follow your OS preference.", icon: <Smartphone className="size-5" /> },
                  { value: "light",  label: t.themeOptionLight,  description: "Always use light mode.", icon: <Sun className="size-5" /> },
                  { value: "dark",   label: t.themeOptionDark,   description: "Always use dark mode.", icon: <Moon className="size-5" /> },
                ] as const).map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex flex-col gap-3 rounded-3xl border-2 p-5 text-left transition-all ${isSelected ? "border-primary bg-primary/5 ring-1 ring-primary/30" : "border-border bg-background hover:border-primary/40"}`}
                    >
                      <div className="flex items-center gap-3">
                        <span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-muted text-muted-foreground">{option.icon}</span>
                        <div>
                          <p className="text-sm font-semibold text-foreground">{option.label}</p>
                          <p className="text-xs text-muted-foreground">{option.description}</p>
                        </div>
                      </div>
                      {isSelected && <span className="absolute top-4 right-4 text-[10px] font-semibold uppercase tracking-[0.24em] text-primary">Selected</span>}
                    </button>
                  )
                })}
              </div>
