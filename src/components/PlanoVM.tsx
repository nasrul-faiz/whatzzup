...
import { Plus, Trash2, ChevronLeft, ChevronRight, Image as ImageIcon, Pencil, MoreVertical, ArrowUp, ArrowDown, Upload, Link, Save, Layers, Search, ChevronDown, Share2 } from "lucide-react"
...
export function PlanoVM({ isSharedView = false }: { isSharedView?: boolean }) {
  ...
  const sharedPageId = useMemo(() => {
    if (!isSharedView) return ""
    const hashParams = new URLSearchParams(window.location.hash.replace(/^#/, ""))
    return hashParams.get("id")?.trim() || ""
  }, [isSharedView])

  const getPageShareUrl = useCallback((pageId: string) => {
    const base = import.meta.env.BASE_URL.replace(/\/$/, "")
    return `${window.location.origin}${base}/#page=plano-vm&shared=plano-vm&id=${encodeURIComponent(pageId)}`
  }, [])

  const copyPageShareLink = useCallback(async (page: PlanoPage) => {
    try {
      await navigator.clipboard.writeText(getPageShareUrl(page.id))
      toast.success(`Share link untuk ${page.name} berjaya disalin`)
    } catch {
  ...
  useEffect(() => {
    if (!isSharedView || !sharedPageId) return
    if (pages.some((page) => page.id === sharedPageId)) {
      setActivePage(sharedPageId)
    }
  }, [isSharedView, sharedPageId, pages])
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="ml-auto flex items-center gap-1">
                    <Button
                      ...
                      onClick={() => void copyPageShareLink(page)}
                      title="Share page"
                      aria-label={`Share ${page.name}`}
                    >
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-2xl border border-border/60 bg-background/70 px-3 py-2 text-xs font-medium text-foreground hover:bg-muted/60"
                >
                  Open Page
                </button>
              </div>
        ...
        <div className="mb-10">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              ...
              {isSharedView && currentPage && (
                <div className="flex h-12 items-center rounded-xl border border-border bg-muted/30 px-4">
                  <span className="text-xs text-muted-foreground">Shared page:</span>
                  <span className="ml-2 truncate text-sm font-medium">{currentPage.name}</span>
                </div>
              )}
            </div>
          </div>
