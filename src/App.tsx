...
function getInitialPageFromHash() {
  ...
  if (hashParams.get("page") === "plano-vm") return "plano-vm"
  return "home"
}
...
function getInitialSharedView() {
  ...
  return (
    ...
    || hashParams.get("shared") === "bot-command"
    || hashParams.get("shared") === "bot-custom-command"
    || hashParams.get("shared") === "plano-vm"
  )
}
      ...
      setHomeRouteDialogPoints(
        Array.isArray(selected.deliveryPoints)
          ? selected.deliveryPoints.map((point: {
              ...
              qrCodes: Array.isArray(point.qrCodes)
                ? point.qrCodes
                    .map((entry) => {
                      const imageUrl = String(entry?.imageUrl ?? "").trim()
                      const destinationUrl = String(entry?.destinationUrl ?? "").trim()
                      const buttonName = String(entry?.buttonName ?? entry?.name ?? entry?.label ?? "").trim()
                    ...
                    .filter((entry): entry is { id: string; imageUrl: string; destinationUrl: string; buttonName: string } => Boolean(entry))
                    .slice(0, 2)
                : undefined,
...
function AppContent() {
  ...
  const renderContent = () => {
    switch (currentPage) {
      ...
      case "plano-vm":
        return <PlanoVM isSharedView={isSharedView} />
      case "gallery-album":
        return <Album />
      ...
      case "bot-custom-command":
        return <BotCustomCommand isSharedView={isSharedView} />
      case "bot-settings":
        return <BotSettings />
      case "home":
      default:
  ...
  const getPageBreadcrumbs = (): { parent?: { label: string }; current: string } => {
    switch (currentPage) {
      ...
      case "settings-appearance-theme":
        return { parent: { label: contentText.breadcrumb.settings }, current: contentText.breadcrumb.theme }
      case "settings-route-colors":
